...
def jesaispasencore():    
    for g in ngt:
        ...
        while lend<leni//2:
            #print(lend)
            first=g.loc[lend, "qty"]
            g.loc[lend,"qty"]=g.loc[lenf, "qty"]
            g.loc[lenf,"qty"]=first
            sec=g.loc[lend, "id_item"]
            g.loc[lend,"id_item"]=g.loc[lenf, "id_item"]
            g.loc[lenf,"id_item"]=sec
            lend+=2
            lenf-=2   
        #print(g.qty)
...
if __name__ == "__main__":
    ...
    #write_data_conc("submission_modif_anthony_new2.csv", co)
